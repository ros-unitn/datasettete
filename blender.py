...
def take_pic(i):
    ...
    shutil.copyfile(bpy.context.scene.render.filepath, bpy.context.scene.render.filepath.replace("images", "images_bbox"))


    with open("labels/img"+str(i)+".txt", "w") as f:
        for name in names:
            if name in bpy.data.objects: # and bpy.data.objects[name].location.z > 0.762: #se esiste ed è nel campo visivo
                ...
                if yolo_x+yolo_w/2 <= 1 and yolo_x-yolo_w/2 >= 0 and yolo_y+yolo_h/2 <= 1 and yolo_y-yolo_h/2 >= 0:
                    f.write(str(names.index(name)) + " " + str(yolo_x) + " " + str(yolo_y) + " " + str(yolo_w) + " " + str(yolo_h) + "\n")
                    print("x,y,w,h:",center_x,center_y,w,h) 

                    # read image
                    img = cv2.imread(bpy.context.scene.render.filepath.replace("images", "images_bbox"))
                    # get contours
                    cv2.rectangle(img, (int(center_x-w/2), int(center_y+h/2)), (int(center_x+w/2), int(center_y-h/2)), (0, 0, 255), 2)
                    cv2.putText(img, name, (int(center_x-w/2), int(center_y-h/2-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
                    # save resulting image
                    cv2.imwrite(bpy.context.scene.render.filepath.replace("images", "images_bbox"), img)
...
for i in range(746, 1501):
    ...
    for name in names:
        if random.choice([True, False]):
            ...
            material = bpy.data.materials.new(name="randcolor"+str(random.randint(0,6000)))
            material.diffuse_color = [random.uniform(0,1),random.uniform(0,1),random.uniform(0,1), 1]  # some rose color
